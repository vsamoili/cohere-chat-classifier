...
class CohereChat:
    ...
    def __init__(self, base_message: Optional[str] = None):
        ...
        self.chat_delimiter = '###'
        if not base_message:
            self.chat_base_message = f"""You are a sentiment analysis classifier. You will be provided a list of 4 movie 
...
"""
    ...
    def create_review_prompt(self, texts: List[str], continuation_prompt: bool = False) -> str:
        if continuation_prompt:
            return f'Do the same for the {len(texts)} following reviews: \n\n{self.chat_delimiter}\n'.join(texts)
        else:
